...
Promise.all([addScript('https://cdn.rawgit.com/AdaRoseEdwards/dirty-dom/v1.1.3/build/dirty-dom-lib.min.js').promise, addScript('https://twemoji.maxcdn.com/2/twemoji.min.js').promise]).then(function () {
	...
	var skinTone = ['', '🏼', '🏿', '🏽', '🏾', '🏻'];
	var skinToneSelector = document.createElement('ul');
	...
	$('#emoji__options-exit').on('click', function () {
		return document.body.classList.remove('options');
	});
	...
	var hammertime1 = new Hammer($('#emoji__options'));
	...
	var hammertime2 = new Hammer($('#emoji__grid-handle'));
	hammertime2.get('swipe').set({ direction: Hammer.DIRECTION_VERTICAL });
	hammertime2.on('swipedown', function () {
		return document.body.classList.remove('emoji-input');
	});
});
}());
